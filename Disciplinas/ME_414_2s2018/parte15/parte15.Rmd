...
O Datafolha quer fazer uma pesquisa de boca-de-urna para predizer o resultado de uma eleição com apenas dois candidatos.  
...
> $$P \left( -\frac{0.01}{\sqrt{Var(\hat{p})}} < \frac{\hat{p} - p}{\sqrt{Var(\hat{p})}} < \frac{0.01}{\sqrt{Var(\hat{p})}} \right)$$
...
* O intervalo de confiança de $p$, com i.c. de $95\%$; interprete o resultado.
...
* O intervalo de confiança de $95\%$ é dado por: $$ \mbox{IC}(p;0.95) = 0.333 \pm 1.96 \sqrt{\frac{0.333 \cdot 0.667}{300}} = 0.333 \pm 0.053$$ Ou simplesmente $(0.280 ; 0.387)$. 
